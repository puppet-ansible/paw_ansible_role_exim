...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_exim"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_exim"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_exim\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_exim"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_exim"
...
$ExtraVars = @{}
if ($env:PT_exim_dc_eximconfig_configtype) {
  $ExtraVars['exim_dc_eximconfig_configtype'] = $env:PT_exim_dc_eximconfig_configtype
}
if ($env:PT_exim_dc_localdelivery) {
  $ExtraVars['exim_dc_localdelivery'] = $env:PT_exim_dc_localdelivery
}
if ($env:PT_exim_primary_hostname) {
  $ExtraVars['exim_primary_hostname'] = $env:PT_exim_primary_hostname
}
